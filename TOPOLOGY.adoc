...
gv-clade-index is the central registry for the hyperpolymath ecosystem,
providing a VeriSimDB-backed taxonomy of 200+ repositories organised
into 12 clades (fv, nl, rm, gv, db, ap, ix, dx, pt, ax, gm, sc). It
exposes a Cloudflare Worker API for dashboards, drift detection, and
work-tracking VCL queries. The index is the canonical map of the
ecosystem — editing it never modifies the repositories it describes.
