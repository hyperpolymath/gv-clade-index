...
*Map and territory*: This registry is the map — queryable, presentable,
always clean. Repositories are the territory. Editing the map never
damages the territory.
...
|`+fv+` |Formal Verification & Proofs |Provable correctness,
reversibility, dependent types

|`+nl+` |Nextgen Languages |Compilers, runtimes, language tooling

|`+rm+` |Repo Management & Tooling |Scaffolding, graph analysis, bot
orchestration

|`+gv+` |Governance & Standards |Licensing, compliance, policy
enforcement
...
|`+pt+` |Protocols & Interop |Internet standards, protocol
implementations
...
* `+pointer+` — value lives in the repo (VeriSimDB caches +
drift-detects)
...
|3 |Cloudflare Worker API (hardened, tested, CI-gated) |In progress
(~60%)
...
Rollout operations — timelines, dependencies, ownership and go/no-go gates
for each phase — live in
`docs/governance/RELEASE-RUNBOOK.adoc`; this table is the summary, the
runbook is the procedure, `.machine_readable/6a2/STATE.a2ml`
`[route-to-mvp]` is the machine-readable status.
