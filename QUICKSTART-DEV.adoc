...
// Replace gv-clade-index, cd worker && npm install, cd worker && npm test, JavaScript (Cloudflare Worker) + A2ML/VQL data + bash tooling with actuals
= gv-clade-index — Quick Start for Developers
:toc:
:toclevels: 2

== Tech Stack

JavaScript (Cloudflare Worker) + A2ML/VQL data + bash tooling
...
12-clade taxonomy; every repo declares one primary clade; the map never mutates the territory.
